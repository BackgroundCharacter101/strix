...
export function makeStrixApi(overrides: StrixApiOverrides = {}): StrixApi {
  const base: StrixApi = {
    ...
    workspace: {
      ...
      saveAs: vi.fn(async (): Promise<string | null> => null),
    ...
    git: {
      ...
      stashPush: vi.fn(async () => ({ ok: true, output: '' })),
      stashPop: vi.fn(async () => ({ ok: true, output: '' })),
      stashApply: vi.fn(async () => ({ ok: true, output: '' })),
      stashDrop: vi.fn(async () => ({ ok: true, output: '' })),
    },
    terminal: {
